...
The blocks `./study plan` draws from. Each targets a specific failure, has a rough cost so a session can be built to fit a budget, and fires on a diagnosis rather than on preference. Warrants are in [research.md](research.md).
...
| **Diagnostic inversion** | symptom to cause | 10m | strength 3 |
...
**Teach-back.** The user explains the concept as if to a competent engineer who has not met it. The gap between feeling able to explain something and actually explaining it is large and invisible from the inside.

**Faded worked example.** A complete worked design, then a variant with parts removed for the user to fill.

**Full problem.** The only block that measures integration, and the only place the **transfer gap** can be observed: the difference between a facet's strength in isolation and its strength when it has to be recognised inside a larger problem.
